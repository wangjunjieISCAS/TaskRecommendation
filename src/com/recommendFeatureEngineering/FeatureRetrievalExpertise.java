...
public class FeatureRetrievalExpertise {
	...
	public HashMap<String, ArrayList<Double>> retrieveExpertiseFeatures ( TestProject project, TestTask task, int recTimePoint, 
		...
		//HashMap<String, Double> mannSimList = this.retrieveManhattanSimilarity( probExpertise, testAdeq);
		
		//HashMap<String, Double> eucSimTestList = this.retrieveEuclideanSimilarityTest(probExpertise, testAdeq);
		//HashMap<String, Double> mannSimTestList = this.retrieveManhattanSimilarityTest( probExpertise, testAdeq);
		...
		HashMap<String, Double> jacSimList3 = this.retrieveJaccardSimilarity(probExpertise, testAdeq, 0.3);
		HashMap<String, Double> jacSimList4 = this.retrieveJaccardSimilarity(probExpertise, testAdeq, 0.5);
		...
		for ( String workerId : probSimList.keySet() ){
			...
			//featureList.add( mannSimList.get( workerId ));
			
			//featureList.add( eucSimTestList.get( workerId ));
			//featureList.add( mannSimTestList.get( workerId ));
	...
	// 计算cosine similarity时，只考虑在testAdeq中出现的词，对于没有在testAdeq中出现，但是也属于worker expertise的，没有考虑（主要影响是在分母中没有考虑）。
	...
	public HashMap<String, Double> retrieveEuclideanSimilarityTest ( HashMap<String, HashMap<String, Double>> probExpertise, HashMap<String, Double> testAdeq ){
		HashMap<String, Double> eucSimList = new HashMap<String, Double>();
	...
	//sum (|xi-yi|), if xi < yi, treat as 0
	public HashMap<String, Double> retrieveManhattanSimilarity ( HashMap<String, HashMap<String, Double>> probExpertise, HashMap<String, Double> testAdeq ){
	...
	public HashMap<String, Double> retrieveManhattanSimilarityTest ( HashMap<String, HashMap<String, Double>> probExpertise, HashMap<String, Double> testAdeq ){
		HashMap<String, Double> manSimList = new HashMap<String, Double>();
