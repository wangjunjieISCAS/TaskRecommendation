...
public class RecTimePoint {
	...
	public ArrayList<Integer> decideRecTimePoint ( TestProject project, int stablePara, int lengthPara ){
		...
		for ( int i =0; i < reportStatusList.size(); i++ ){
			if ( reportStatusList.get(i).equals( Boolean.TRUE )){
		...
		*/
		...
		//output for another experiment
		try {
			BufferedWriter writer = new BufferedWriter ( new OutputStreamWriter( new FileOutputStream ( new File ("data/output/nonYieldingWindow.csv") , true ) ) );
			writer.write( project.getProjectName() + ",");
			for ( Integer beginIndex : stableInfoList.keySet() ){
				Integer length = stableInfoList.get( beginIndex );
				if ( length < lengthPara )
					continue;
				
				Integer firstValidBug = beginIndex + length;
				writer.write( firstValidBug + ",");
			}
			writer.newLine();
			writer.flush();
			writer.close();
	...
	public static void main ( String[] args ){
		...
		*/
		...
		TestProjectReader reader = new TestProjectReader();
		ArrayList<TestProject> projectList = reader.loadTestProjectList( "data/input/projects-sentence");
		for ( int i =0; i < projectList.size(); i++ ){
			TestProject project = projectList.get( i );
			pointTool.decideRecTimePoint(project, 5, 10);
		}
	}
}
